...
export default function EmployeeProjectAdd() {
    ...
    useEffect(() => {
        ...
        if (projectId) {
            employeeProjectService.getProjectById(projectId)
                .then((response) => {
    ...
    const handleSubmit = (values) => {
        ...
        employeeProjectService.add(values)
            .then((res) => {
                console.log("Response từ server:", res);

                if (res?.data?.StatusCode === 5000) {
                    console.log("Đã tồn tại");
                    toast.error(res.data.message || "Employee already assigned to this project!");
                } else {
                    console.log("Ok");
